...
Businesses face a choice when it comes to implementing Business Intelligence (BI) tools. Some opt for the bundled BI tools within individual applications, while others develop enterprise-wide BI application layers that consolidate data from multiple sources to generate comprehensive insights. However, many of these tools come with a steep learning curve, demanding technical expertise for developing reports and dashboards. This presents a significant challenge because the primary consumers of these BI assets are business users who often lack the technical background and skill set to create these BI artifacts, relying on IT professionals.

This project aims to simplify the BI reporting process by developing an application that seamlessly integrates with multiple backend databases, extracts the required data, and presents it through graphs, reports, dashboards, and more. While the application will be compatible with multiple databases, the project will focus on one or two open-source databases as proof of concept (e.g., PostgreSQL and MySQL). T
